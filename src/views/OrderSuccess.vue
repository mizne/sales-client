  ...
  <div class="order-success-container">
    ...
    <deal-content v-if="orderDetail">
      ...
      <div class="order-detail">
        ...
        <delivery v-if="needDeliveryFee" :delivery-distance="deliveryDistance" :delivery-fee-value="deliveryFeeValue" 
        :delivery-time="deliveryTime"></delivery>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      // 主页 有订单情况下 则直接进入
      // 订单失败页面 下订单失败则继续下单
      if (from.name === 'ShoppingCart' || from.name === 'Alipay' || from.name === 'PhoneVerify' || from.name === 'Home' || from.name === 'OrderFailed') {

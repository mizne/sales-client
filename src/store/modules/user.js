  ...
  FETCH_DELIVERY_FEE: ({ commit, dispatch, rootState, state }) => {
    ...
    ;(state.userLatitude && state.userLongitude
      ...
      Logger.info({
        ...
        description: `userLatitude: ${state.userLatitude}, userLongitude: ${state.userLongitude}, distance: ${distance} 米`
      })

      UserService.getDeliveryFee(distance)
        .then(
          ({ deliveryFeeId, deliveryFeeValue, deliveryTime, startPrice }) => {
